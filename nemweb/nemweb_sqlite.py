...
def start_from(
    ...
    Args:
        ...
        timestamp_col (str, optional): the column to use for the timestamp.
            Defaults to 'SETTLEMENTDATE'
        start_date (str, optional): the start date to fall back on
    ...
    """
    try:
        dt = table_latest_record(table_name,
                                 db_name=db_name,
        ...
        if start_date is None:
            dt = datetime.datetime.strptime('19960101', '%Y%m%d')
        else:
...
def db_path(db_name):
    """ Provides the path to the database, based on configuration and db_name.
